...
- Fourth level QA finding resolved: `social-progress-0..8` are now distinct same-layout raster states. Zhou Qiming starts in a believable sofa/bedside scrolling pose and gradually cools down while all eight clue bodies remain present.
...
- Fourth level QA finding resolved: after the social-v2 asset swap, all eight hotspot centers were recalibrated from the visible object groups. Playwright screenshots were regenerated and manually checked for the 8/8 counter, distributed clue placement, and completion-state expression change.
