...
export class RooftopScene extends Phaser.Scene {
  ...
  private addClueObjects(): void {
    for (const hotspot of this.rooftopData.scene.hotspots) {
      ...
      this.tweens.add({ targets: image, y: y - 3, duration: 1600 + Number(hotspot.id.slice(1)) * 140, yoyo: true, repeat: -1, ease: "Sine.inOut" });
    }
  }
  ...
  private addFoundMarkers(): void {
    for (const hotspot of this.rooftopData.scene.hotspots) {
      ...
      pin.fillCircle(0, 0, 17);
      pin.strokeCircle(0, 0, 17);
  ...
  private addStampEffect(hotspot: SceneHotspot): void {
    const rect = this.getHotspotRect(hotspot);
    const stamp = this.add.text(rect.x + rect.width * 0.16, rect.y + rect.height * 0.12, "嘴硬截获", {
      color: "#c64628",
      ...
      fontStyle: "bold",
      padding: { x: 9, y: 6 }
    }).setOrigin(0.5).setAngle(-7).setDepth(52).setScale(1.42);
  ...
  private addCoolingFurnace(): void {
    ...
    const progress = getOfficeProgress(this.rooftopData.scene, this.rooftopData.foundHotspotIds);
    const machine = this.add.image(1035, 528, "rooftop-cooling-furnace").setDepth(27);
    machine.setDisplaySize(186, 155);
  ...
  private addMachinePulse(): void {
    const pulse = this.add.rectangle(1035, 528, 176, 128, 0xf3c45b, 0.22).setDepth(31);
    this.tweens.add({ targets: pulse, alpha: 0, scale: 1.28, duration: 650, ease: "Cubic.out" });
  }
  ...
  private getClueDepth(hotspot: SceneHotspot): number {
    return hotspot.id === "h5" ? 19 : 26;
  }

  private getClueAngle(hotspot: SceneHotspot): number {
    const angles: Record<string, number> = { h1: -5, h2: 4, h3: 3, h4: -4, h5: 2 };
