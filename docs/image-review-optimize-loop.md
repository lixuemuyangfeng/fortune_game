...
- From level four onward, the accepted image must include same-category decoy clusters. A clue should usually be a subpart or relationship inside the cluster: a red-stamped bill corner under a cable, a key tooth on an envelope corner, a colored bookmark edge among other tabs, or a small send arrow on a phone. If the answer is the center of a whole large paper, whole laptop, whole phone, or whole poster, the level is too easy unless that object is surrounded by equally plausible false targets.
- Before accepting a clue, run the "clear object, delayed meaning" check: the object/interface must be visually recognizable at game size, but the reason it is evidence should come from context. If a clue is a digital behavior such as a group invite, pinned comment, unsent reply, or comment thread, put it on a plausible screen/interface or a clearly shown draft surface; do not turn it into an unexplained paper card.
